class PackItemsController < ApplicationController
	...
	def index
		@pack_items = PackItem.where("pack_id" => 1)
	end
	def new
...
def pack_item_params
  params.require(:pack_item).permit(:name, :description, :post_summary, :event_id, :gear_id, :user_id, :wearing, :delivery, :category_id, :highlight)
end
